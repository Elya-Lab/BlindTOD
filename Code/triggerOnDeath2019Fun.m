...
%             end
            ...
            %% Calculate average and error for all groups

            disp(['Calculating mean and error...(', num2str(round(toc)),' sec elapsed)']);

            for g=(1:length(data.feat)) 
                for h=(1:length(data.rXes))
                    if ~isempty(data.(data.rXes{h}).(data.feat{g}).toDmatrix)
                        if size(data.(data.rXes{h}).(data.feat{g}).toDmatrix,2) > 1
                            % Calculate average of each variable for todMatrix, excluding NaN values
                            data.(data.rXes{h}).(data.feat{g}).Allavg = ...
                                nanmean( transpose( data.(data.rXes{h}).(data.feat{g}).toDmatrix(1:data.params.lastFrames,:)));
                            
                            % Calculate standard error (SE) of each variable, excluding NaN
                            % values
                            data.(data.rXes{h}).(data.feat{g}).Allse = ...
                                nanstd(transpose(data.(data.rXes{h}).(data.feat{g}).toDmatrix(1:data.params.lastFrames,:))) / ...
                                sqrt(size(data.(data.rXes{h}).(data.feat{g}).toDmatrix,2));
                        
                        else % if there's only one fly of given type, don't average (prevent graphing error later on)
                            
                            % Calculate average of each variable for todMatrix, excluding NaN values
                            data.(data.rXes{h}).(data.feat{g}).Allavg = ...
                                data.(data.rXes{h}).(data.feat{g}).toDmatrix(1:data.params.lastFrames,:);
                        ...
                        % Save smoothed version of avg
                        data.(data.rXes{h}).(data.feat{g}).avg = ...
                            downsample( smoothdata( data.(data.rXes{h}).(data.feat{g}).Allavg,...
                            'movmean',data.params.avgWin),data.params.downSamp);

                        % Saved smoothed version of se
                        data.(data.rXes{h}).(data.feat{g}).se = ...
                            downsample(smoothdata(data.(data.rXes{h}).(data.feat{g}).Allse,...
                            'movmean',data.params.avgWin),data.params.downSamp);

                        % Replace any NaN values in SE and avg vector with 0 to prevent
                        % areaBar from encountering graphing error
                        data.(data.rXes{h}).(data.feat{g}).avg(isnan(data.(data.rXes{h}).(data.feat{g}).avg))=0;
                        data.(data.rXes{h}).(data.feat{g}).se(isnan(data.(data.rXes{h}).(data.feat{g}).se))=0;

                        % For plotting all ROIs along with avg data - save
                        % all smoothed ROI data
                        data.(data.rXes{h}).(data.feat{g}).smoothdata = ...
                            downsample( smoothdata( data.(data.rXes{h}).(data.feat{g}).toDmatrix(1:data.params.lastFrames,:),...
                            1,'movmean',data.params.avgWin),data.params.downSamp);

                    end    
                end
            end

            %% Plot average vars for all groups
            data.xspace = 1:1:data.params.lastFrames;
            data.xspace = downsample(data.xspace,data.params.downSamp);
            
            if data.params.doPlot
                disp(['Plotting results...(', num2str(round(toc)),' sec elapsed)']);

                key = cell(1,length(data.rXes));
                colorct=1;
                ...
                for h=(1:length(data.rXes))
                    for sPlot=1:2
                        if ~isempty(data.(data.rXes{h}).ypos.toDmatrix)
                            subplot(2,1,sPlot);
                            hold on;

                            areaBar(data.xspace,... 
                                    data.(data.rXes{h}).(data.feat{sPlot}).avg, ...
                                    data.(data.rXes{h}).(data.feat{sPlot}).se,...
                                    data.graph.colors{colorct+1}, data.graph.colors{colorct+1},...
                                    '-',0.1);
                        end
                    end
                    if ~isempty(data.(data.rXes{h}).ypos.toDmatrix)
                    ...
                    if colorct > length(data.graph.colors)
                        colorct = 1;
                    end
                end
                
                % Remove any empty key entries
                key = key(~cellfun(@isempty,key));

                subplot(2,1,1);
                ylabel('Y position (height in vial)','fontsize',data.graph.labelFont);
                ...
                set(gca,'xticklabel', round(data.params.hoursBeforeDeath:-1*data.graph.xInt:0,1), 'fontsize', data.graph.tickFont);
                xlim([1 data.params.lastFrames]);
                lgd = legend(key,'Location','best');
                legend boxoff;
                lgd.FontSize = data.graph.legFont;

                title('Mean +/- SE position, speed','fontsize',data.graph.titleFont);
                ...
                % Add suptitle and save figure
                s = sgtitle(data.graphTitle);
                s.HorizontalAlignment='center';


                set(gcf,'PaperUnits','inches','PaperPosition',[0 0 10 8]);
                print(strcat(data.outDir,data.filePre,'_triggered on death.png'),'-dpng');
                print(strcat(data.outDir,data.filePre,'_triggered on death.pdf'),'-dpdf');
                close();
            end
            
            %% Plot all cads      
            % Overlay all cadaver traces (y position and speed) on same plots along with average 
            if data.params.plotAllCads && count.Cadavers >0
                try
                    plotAllCads(data);
                catch
                end
            ...
            %% Save average info in struct and write to .mat file
            outfile = strcat(data.outDir,genoListSafe{b},'_',data.sexes{i},'_last ',num2str(data.params.hoursBeforeDeath),' hours.mat')
            disp(strcat('Outfile = ',outfile));
            if data.params.saveData
                save(outfile,'data','-mat','-v7.3');
            end
            
            %% Plot all group avgs on local time (i.e. not aligned to time of death)
            plotLocalFun(data,phaseShift);  
            
            disp(strcat('Done with ',genoList(b),', ',data.sexes{i},'!', newline, num2str(round(toc)),' sec elapsed'));
        end
    end
end